...
def my_check_mandatory_constraints(timetable: {str: {(int, int): {str: (str, str)}}}, timetable_specs: dict):
    '''
            ...
            for room in timetable[day][interval]:
                if timetable[day][interval][room]:
                    ...
                    if prof in profs_in_crt_interval:
                        ...
                        constangeri_dict[constrangeri_incalcate]=(('2MateriiDifAcProfSimultan', prof, subject, day, interval, room))
                    else:
                    ...
                    if subject not in timetable_specs[SALI][room][MATERII]:
                        ...
                        constangeri_dict[constrangeri_incalcate]=(('SalaNepotrivitaPtMaterie', prof, subject, day, interval, room))
                        constrangeri_incalcate += 1
                    ...
                    if subject not in timetable_specs[PROFESORI][prof][MATERII]:
                        ...
                        constangeri_dict[constrangeri_incalcate]=(('ProfNepotrivitPtMaterie', prof, subject, day, interval, room))
                        constrangeri_incalcate += 1
    ...
    for subject in acoperire_target:
        if acoperire_reala[subject] < acoperire_target[subject]:
            #print("Materia " + str(subject) + " nu are acoperirea necesară! " + str(
            #    acoperire_reala[subject]) + ", trebuie: " + str(acoperire_target[subject]))
            constangeri_dict[constrangeri_incalcate]=(('MaterieNeacoperita', None, subject, None, None, None))
            constrangeri_incalcate += 1
    ...
    for prof in ore_profesori:
        if ore_profesori[prof] > 7:
            ...
            constangeri_dict[constrangeri_incalcate] = (('ProfFrecventaPreaMare', prof, None, None, None, None))
            constrangeri_incalcate += 1

    return constrangeri_incalcate, constangeri_dict
...
def my_optional_constraints_checker(timetable: {str: {(int, int): {str: (str, str)}}}, timetable_specs: dict):
    '''
    Se verifică dacă orarul generat respectă cerințele profesorilor pentru a fi un orar valid.
    '''

    constrangeri_incalcate = 0
    constrangeri_incalcate_dict = {}
                        ...
                        for interval in timetable[day]:
                            for room in timetable[day][interval]:
                                if timetable[day][interval][room]:
                                    ...
                                    if prof == crt_prof:
                                        constrangeri_incalcate_dict[prof] = [(day, None), (day, str(interval))]
                                ...
                                for room in timetable[day][interval]:
                                    if timetable[day][interval][room]:
                                        ...
                                        if prof == crt_prof:
                                            ...
                                            constrangeri_incalcate_dict[prof] = [(None, str(interval)),
                                                                                 (day, str(interval))]
                                            constrangeri_incalcate += 1

    return constrangeri_incalcate, constrangeri_incalcate_dict
